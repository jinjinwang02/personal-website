...
window.addEventListener("scroll", () => {
  ...
  circle.style.setProperty(
    "-moz-transform",
    "translate(-50%, -50%) rotate(" + offset + "deg)"
  );
  circle.style.setProperty(
    "-webkit-transform",
    "translate(-50%, -50%) rotate(" + offset + "deg)"
...
const changeImages = (total, index) => {
  ...
  myInterval = setInterval(() => {
    if (i < total) {
      projectsCursor.style.setProperty(
        "background-image",
        `url(public/images/project-${index}-${i}.jpg)`
      );
      i++;
    } else {
      i = 1;
  ...
  document.querySelectorAll(".projects__project-container").forEach((b) =>
...
document.querySelector(".projects__2").addEventListener("mouseenter", () => {
  projectsCursor.style.setProperty(
    "background-image",
    "url(public/images/project-2-1.jpg)"
  );
  setImgSize(53, 30);
});
